/**
 * Certificate Archive Management JavaScript
 * Handles form interactions, dynamic field visibility, and data loading
 */
document.addEventListener("DOMContentLoaded", () => {
    // Elements
    const restoreButtons = document.querySelectorAll(".btn-icon.restore-cert")
    const restoreModal = document.getElementById("restoreCertModal")
    ...
    const restoreModalMessage = document.getElementById("restoreModalMessage")
    ...
    // Open modal when restore button is clicked
    if (restoreButtons.length > 0) {
      restoreButtons.forEach((button) => {
        button.addEventListener("click", function (e) {
          ...
          const certName = this.getAttribute("data-certificate-name")
  
          // Update modal message
          if (restoreModalMessage) {
            restoreModalMessage.textContent = `Are you sure you want to restore ${certName || "this certificate"}?`
          }
          ...
          setTimeout(() => {
            if (cancelRestoreBtn) cancelRestoreBtn.focus()
          }, 100)
        })
      })
    }
    ...
    function closeModal() {
      if (restoreModal) {
        ...
        restoreModal.classList.add("hiding")
  
        // Remove hiding class after animation completes
        setTimeout(() => {
          restoreModal.classList.remove("hiding")
    ...
    if (restoreModal) {
      restoreModal.addEventListener("keydown", (e) => {
        if (e.key === "Tab") {
          const focusableElements = restoreModal.querySelectorAll(
